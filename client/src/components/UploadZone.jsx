...
export default function UploadZone({ onAnalyze, loading }) {
  ...
  const { inputRef, file, openFilePicker, handleFileChange, handleDrop } = useFileUpload();
    ...
    <section className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.05fr_0.95fr]">
      ...
      >
        <div className="mb-6 flex flex-wrap gap-3">
          {fileTypeBadges.map((type) => (
            ...
            >
              <span className="rounded-full bg-cyan/10 px-2 py-1 text-cyan">{type.icon}</span>
              {type.label}
            </span>
          ))}
        </div>
        ...
        >
          ...
          <p className="mt-3 max-w-xl text-base text-slate-300">
            Click to browse or drag a PDF, DOCX, or TXT file. Everything is processed in-memory for privacy.
          </p>
        ...
        {file ? (
          <div className="mt-5 rounded-3xl border border-emerald/20 bg-emerald/10 p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.24em] text-emerald/80">Selected file</p>
                <p className="mt-1 text-lg font-semibold text-white">{file.name}</p>
              </div>
              <p className="text-sm text-emerald-100">{formatFileSize(file.size)}</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="mt-6">
          ...
          <textarea
            ...
            maxLength={4000}
            onChange={(event) =>
              dispatch({ type: "SET_JOB_DESCRIPTION", payload: event.target.value })
            }
      ...
      >
        <div className="glass-panel rounded-[2rem] p-6 md:p-8">
          <p className="text-sm uppercase tracking-[0.28em] text-cyan/70">What you get</p>
          <ul className="mt-5 space-y-4 text-slate-200">
            <li className="rounded-3xl border border-white/8 bg-white/[0.03] p-4">
              ATS scorecards with section-by-section grading and benchmark context.
            </li>
            <li className="rounded-3xl border border-white/8 bg-white/[0.03] p-4">
              Skill gap analysis, missing keywords, and role-fit scoring against a job description.
            </li>
            <li className="rounded-3xl border border-white/8 bg-white/[0.03] p-4">
              Rewrite-ready improvements for summary, experience, and skills plus a downloadable report.
            </li>
          </ul>
        </div>

        <div className="glass-panel rounded-[2rem] p-6 md:p-8">
          <p className="text-sm uppercase tracking-[0.28em] text-emerald/75">Previous analyses</p>
          <div className="mt-5 space-y-3">
            {state.history.length ? (
              state.history.map((entry) => (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <p className="font-medium text-slate-100">{entry.fileName}</p>
              ...
              <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.03] px-4 py-5 text-sm text-slate-400">
                Your recent analyses will appear here for quick reference on this device.
              </div>
            )}
          </div>
        </div>
